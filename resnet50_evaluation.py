"""
ResNet50 Model Evaluation and Visualization
...
# Configuration
MODEL_PATH = 'models/resnet50_asl.keras'  # Change to .h5 if needed
...
def evaluate_model(model, test_generator):
    ...
    with open('results/resnet50_classification_report.txt', 'w') as f:
        f.write(f"ResNet50 Model Evaluation Results\n")
        f.write("="*60 + "\n\n")
...
def plot_confusion_matrix(y_true, y_pred):
    ...
    plt.title('ResNet50 - Confusion Matrix', fontsize=20, fontweight='bold')
    plt.xlabel('Predicted Label', fontsize=14)
    plt.ylabel('True Label', fontsize=14)
    plt.xticks(rotation=45, ha='right')
    plt.yticks(rotation=0)
    plt.tight_layout()
    plt.savefig('results/resnet50_confusion_matrix.png', dpi=300, bbox_inches='tight')
    print("✓ Confusion matrix saved: results/resnet50_confusion_matrix.png")
    plt.close()
...
def plot_accuracy_per_class(y_true, y_pred):
    ...
    plt.title('ResNet50 - Per-Class Accuracy', fontsize=18, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig('results/resnet50_class_accuracy.png', dpi=300, bbox_inches='tight')
    print("✓ Per-class accuracy plot saved: results/resnet50_class_accuracy.png")
    plt.close()
...
def plot_top_errors(y_true, y_pred):
    ...
    bars = ax.barh(labels, counts, color='coral', edgecolor='darkred')
    ax.set_xlabel('Number of Misclassifications', fontsize=12)
    ax.set_title('ResNet50 - Top 10 Misclassification Pairs', fontsize=16, fontweight='bold')
    ax.invert_yaxis()
    ...
    plt.tight_layout()
    plt.savefig('results/resnet50_top_errors.png', dpi=300, bbox_inches='tight')
    print("✓ Top errors plot saved: results/resnet50_top_errors.png")
    plt.close()
...
def plot_confidence_distribution(predictions, y_true, y_pred):
    ...
    plt.title('ResNet50 - Confidence Score Distribution', fontsize=16, fontweight='bold')
    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig('results/resnet50_confidence_distribution.png', dpi=300, bbox_inches='tight')
    print("✓ Confidence distribution saved: results/resnet50_confidence_distribution.png")
    plt.close()
